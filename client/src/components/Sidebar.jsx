...
const Sidebar = ({ isMenuOpen, setIsMenuOpen }) => {
  ...
  return (
    <div className={`flex flex-col h-screen min-w-72 p-4 dark:bg-gradient-to-b from-
  ...
  transition-all duration-500 max-md:absolute left-0 z-1 ${!isMenuOpen && 'max-md:-translate-x-full'}`}>
        ...
        transition-transform duration-150 ease-in-out active:scale-97'>
     ...
     {chats.length > 0 && <p className='mt-4 text-sm'>Recent Chats </p>} 
      ...
      rounded-md cursor-pointer hover:scale-103 transition-all'>
        ...
        <div className='flex flex-col text-sm'>
          <p>Credits : {user?.credits ?? getStoredCredits()}</p>
          <p className='text-xs text-gray-400'>Purchase credits to use ravion ai</p>
        </div>
      </div>
